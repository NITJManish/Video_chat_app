...
const joinRoom=async ()=>{
  const responseMessage=await getRoomExists(roomIdValue);
    ...
    if(roomExists){
      if(full){
        setErrorMessage("meeting is full. Please try agains later");
      }else{
        //join a room !
        //save in our redux store meeting is which was provided by user which would like to join
        setRoomIdAction(roomIdValue);
        navigate("/room");

      }
    }else{
      setErrorMessage("meetng not found. check your meeting id");
    }
}
...
export default connect(mapStoreStateToProps,mapActionsProps)(JoinRoomContent);
